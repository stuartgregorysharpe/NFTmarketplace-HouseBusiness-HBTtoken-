...
contract HouseStaking {
    ...
    // stake House Nft
    function stake(uint256 _tokenId, uint256 _stakingType, address _owner) external {
        ...
        houseNFT.transferFrom(_owner, address(this), _tokenId);
        ...
        emit NFTStaked(_owner, _tokenId, _stakingType, block.timestamp);
    }

    // Unstake House Nft
    function unstake(uint256 _tokenId, address _owner) external {
        require(_tokenId > 0, 'Invalid Token ID');
        StakedNft memory unstakingNft;
        uint256 counter;
        for (uint256 i = 0; i < stakedNfts[_owner].length; i++) {
            if (stakedNfts[_owner][i].tokenId == _tokenId) {
        ...
        IERC721(houseNFTAddress).transferFrom(address(this), _owner, _tokenId);

        IHouseBusiness(houseNFTAddress).setHouseStakedStatus(_tokenId, false);
        stakedCounter--;

        emit NFTUnstaked(_owner, _tokenId, unstakingNft.startedDate);
    }
